...
const product = createSlice({
	...
	reducers: {
		addProduct: (state, action) => {
			const idf = state.productList.find((item) => item.id === action.payload.id);
			if (!idf) {
				return {
					...state,
					count: state.count + 1,
		...
		incrementPrice: (state, action) => {
			return {
				...state,
				productList: state.productList.map((item) => item.id === action.payload.id
					? {
						...item,
		...
		summation: (state) => {
			return {
				...state,
				totalPrice: state.productList.reduce((acc, item) =>
					acc + item.userPrice,
					0
				),
			};
